...
def about_screen(stdscr):
    ...
    fields = config.get("fields", ["OS", "Hostname", "CPU", "RAM", "Uptime", "Battery", "Theme", "Shell"])
    ...
    while True:
        ...
        for i, line in enumerate(ascii_art):
            x = max(0, (w - len(line)) // 2)
            try:
                stdscr.addstr(art_start + i, x, line, curses.color_pair(COLOR_NORMAL) | curses.A_BOLD)
            except curses.error:
                pass
        ...
        if key in (ord('q'), ord('Q'), 27, 9):
            ...
            about_edit_menu(stdscr, config)
            config = load_about()
            info = get_system_info(config.get("fields", fields))

ALL_FIELDS = ["OS", "Hostname", "CPU", "RAM", "Uptime", "Battery", "Theme", "Shell", "Python"]

def about_edit_menu(stdscr, config):
    while True:
        result = run_menu(stdscr, "Edit About", ["Edit ASCII Art", "Toggle Fields", "---", "Back"])
        if result == "Back":
            break
        elif result == "Edit ASCII Art":
            line = curses_input(stdscr, "Enter ASCII art line (blank to finish):")
            if line:
                art = config.get("ascii", [])
                art.append(line)
                config["ascii"] = art
                save_about(config)
                curses_message(stdscr, "Line added. Re-enter to add more.")
        elif result == "Toggle Fields":
            fields = config.get("fields", ALL_FIELDS[:])
            while True:
                choices = [f"{'[x]' if f in fields else '[ ]'} {f}" for f in ALL_FIELDS] + ["---", "Back"]
                toggle = run_menu(stdscr, "Toggle Fields", choices, subtitle="Select to toggle on/off")
                if toggle == "Back":
                    break
...
def settings_menu(stdscr):
    ...
    while True:
        ...
        result = run_menu(stdscr, "Settings Menu",
                          ["About", "Theme", "Edit Menus", bootup_label, sound_label, "---", "Back"])
        if result == "Back":
            break
        elif result == "About":
            about_screen(stdscr)
